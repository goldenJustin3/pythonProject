...
class MyBox(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(orientation="vertical", padding=10, spacing=20, **kwargs)
    ...
    def textchanged(self, widget, value):
        if int(value) >= 10 and int(value) < 150:
            self.label.text = "Ваш возраст? " + str(value) + ", " + str(value) + "? Точно?"
        else:
            self.label.text = "Ваш возраст? " + str(value) + " пожалуйста отвечайте серьёзно."
